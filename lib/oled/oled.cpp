...
bool initOLED() {                   // 初始化OLED显示屏的函数
    I2C_OLED.begin(OLED_SDA, OLED_SCL);  // 初始化I2C总线，设置SDA和SCL引脚
    
    if(!display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDRESS)) {  // 尝试初始化显示器
...
void updateDisplay() {              // 更新显示内容的函数
    switchDisplayPage();            // 尝试切换显示页面
    
    switch(currentPage) {           // 根据当前页面类型来更新显示内容
...
void updateSensorPage() {           // 更新传感器页面的函数
    display.clearDisplay();         // 清除显示内容
...
void updateWeatherPage(const WeatherData &weather) {  // 更新天气页面的函数，接收天气数据参数
    display.clearDisplay();         // 清除显示内容
    display.setTextColor(SSD1306_WHITE);  // 设置文本颜色为白色
...
bool switchDisplayPage() {           // 切换显示页面的函数
    unsigned long currentTime = millis();  // 获取当前时间
    
    if(currentTime - lastPageSwitchTime >= PAGE_SWITCH_INTERVAL) {  // 如果距离上次切换已过指定时间间隔
        switch(currentPage) {        // 根据当前页面切换到下一个页面
            case SENSOR_PAGE:        // 如果当前是传感器页面
...
void showStartupScreen() {           // 显示启动画面的函数
    display.clearDisplay();          // 清除显示内容
...
void showError(const char* errorMsg) {  // 显示错误信息的函数
    display.clearDisplay();          // 清除显示内容
    display.setTextSize(1);          // 设置文本大小为1
    display.setTextColor(SSD1306_WHITE);  // 设置文本颜色为白色
    display.setCursor(0, 0);         // 设置光标位置到左上角
